...
public class schoolReportService {
    ...
    */
    ...
    public String[][] library_report_add_times() {
        List<library_report_month_mankinds> library_report_add_times = schoolReportdao.getlibrary_report_add_times();
        String[][] lines = new String[library_report_add_times.size()][4];
        for (int i = 0; i < library_report_add_times.size(); i++) {
            String[] items = new String[4];
            items[0] = String.valueOf(library_report_add_times.get(i).getYear());
            items[1] = library_report_add_times.get(i).getPrint_type();
            items[2] = library_report_add_times.get(i).getPrint_location();
            items[3] = String.valueOf(library_report_add_times.get(i).getTimes());
            lines[i][0] = items[0];
            lines[i][1] = items[1];
            lines[i][2] = items[2];
            lines[i][3] = items[3];
